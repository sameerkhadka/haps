                ...
                <div class="panel panel-bordered">
                    ...
                    <form role="form" class="form-edit-add" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="panel-body">
                            ...
                            <!-- Faqs -->
                            <div class="form-group  col-md-12 ">
                                <label class="control-label" for="name">Faqs</label>
                                <table @class('table table-bordered') style="margin-bottom: 0px">
                                    <template>
                                        <tbody is="draggable" tag="tbody" v-model="formData.faqs.body"
                                            v-on:start="drag=true" v-on:end="drag=false" handle=".handle" v-bind="dragOptions">
                                            <transition-group type="transition" :name="!drag ? 'flip-list' : null">
                                                <tr class="profile" v-for="(item,index) in formData.faqs.body">
                                                    <td><span class="handle btn btn-info"><i class="voyager-handle"></i></span>
                                                    </td>
                                                    <td style="width:25%;"><textarea placeholder="Title" type="text" :ref="`Faq${index+1}`"
                                                            class="form-control" rows="4" cols="30"
                                                            v-model="item.question"></textarea></td>
                                                    <td style="width:60%;">
                                                        <vue-ckeditor :editor="editor" v-model="item.answer"
                                                            :config="editorConfig">
                                                        </vue-ckeditor>
                                                    </td>
                                                    <td style="width:5%;"><a tabindex="-1" class="btn btn-danger"
                                                            style="margin-top: 4px" v-on:click="deleteFaq(index)">x</a>
                                                    </td>
                                                </tr>
                                            </transition-group>
                                        </tbody>
                                ...
                                <a tabindex="-1" class="btn btn-success" v-on:click="addFaq">Add Faq
                                    {{--@{{ formData.itinerary.title }}--}}</a>
...
<script>
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            addFaq(e) {
                e.preventDefault();
                this.formData.faqs.body.push({
                    question: '',
                    answer: ''
                })
                this.$nextTick(() => {
                    this.$refs[`Faq${this.formData.faqs.body.length}`][0]
                        .focus()
                })
            },
            deleteFaq(index) {
                this.formData.faqs.body.splice(index, 1);
                this.$nextTick(() => {
                    this.$refs[`Faq${index+1}`][0].focus()
                })
            },
            submitForm(e) {
                e.preventDefault();
                ...
                axios.post('/admin/submit-faqs', formData).then(function(res) {
